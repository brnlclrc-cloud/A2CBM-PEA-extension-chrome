...
// @version      1.1.6
// @description  Synchronisation à la demande de votre Assurance Vie BNP Paribas vers PEA Pro.
// @author       Bruno
// @match        https://*.mabanque.bnpparibas/*
// @match        https://*.bnpparibas.com/*
// @match        https://a2cbm-pea-backend-prod.onrender.com/*
// @match        https://a2cbm-pea-backend-qs4v.onrender.com/*
// @match        https://*.vercel.app/*
// @match        http://localhost:*/*
// @updateURL    https://raw.githubusercontent.com/TON_USER/TON_REPO/main/pea-pro-bnp.user.js
// @downloadURL  https://raw.githubusercontent.com/TON_USER/TON_REPO/main/pea-pro-bnp.user.js
// @grant        GM_getValue
// @grant        GM_setValue
...
(function() {
    ...
    // ==========================================
    // 1. CAPTURE DU TOKEN SUR L'APP PEA PRO (S'exécute automatiquement)
    // ==========================================
    ...
    if (isPeaApp) {
        ...
        return; // On arrête l'exécution ici si on est sur l'app.
    }

    // ==========================================
    // 2. LOGIQUE D'EXTRACTION (Ex-popup.js v1.1.4)
    // ==========================================
    ...
    // ==========================================
    // 3. FONCTION DE DÉCLENCHEMENT (Dormante)
    // ==========================================
    function lancerInterfaceEtExtraction() {
        // Empêche d'ouvrir l'interface en double si on clique plusieurs fois
        if (document.getElementById('pea-pro-tampermonkey-root')) {
            console.log("Interface PEA Pro déjà ouverte.");
            return;
        }

        console.log("🚀 Lancement manuel de l'extraction PEA Pro...");
        ...
        shadow.innerHTML = `
          <style>
            * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
            ...
            .close-btn { color: #64748B; cursor: pointer; padding: 0 5px; }
            .close-btn:hover { color: #FFFFFF; }
          </style>

          <div class="panel" id="mainPanel">
            <div class="header">
              <div class="title" id="toggleCollapse"><span>📈</span> PEA Pro Sync</div>
              <div>
                  <span class="version">v1.1.6</span>
                  <span class="close-btn" id="closePanel">✖</span>
              </div>
            </div>
            
            <div id="panelContent">
                <div class="env-toggle">
                  <button id="btnProd" class="env-btn active">🚀 Prod</button>
                  <button id="btnTest" class="env-btn">🧪 Dev</button>
                </div>

                <div id="dataCard" class="card">
                  <div class="row">
                    <span class="label">Contrat</span>
                    <span id="contractName" class="value" style="font-size:11px;max-width:210px;">Analyse en cours...</span>
                  </div>
                  <div class="row">
                ...
                <div id="tokenSection" style="display:none;margin-top:12px;padding:10px;background:#0F172A;border-radius:10px;border:1px solid #334155;">
                  ...
                  <input type="text" id="tokenField" placeholder="Collez votre token ici..." class="token-input">
                ...
                <div id="toggleSettings" class="settings-link">⚙️ Jeton</div>
                <div id="toggleDebug" class="settings-link" style="color:#38bdf8;margin-top:6px;">🔍 Texte détecté</div>
        ...
        $('#toggleCollapse').onclick = () => { $('#mainPanel').classList.toggle('minimized'); };
        $('#closePanel').onclick = () => { uiContainer.remove(); };
        ...
        // Lancement immédiat de l'extraction car la page est censée être déjà chargée
        const now = new Date();
        ...
        try {
            ...
            if (extractedData && extractedData.totalVal > 0) {
                ...
                $('#contractName').innerText = "Aucune donnée trouvée sur cette page.";
            }
        } catch (err) {
        ...
        $('#syncBtn').onclick = () => {
            ...
            syncBtn.innerHTML = `<span>⏳</span> Synchro...`;
            $('#statusAlert').style.display = 'none';

            const token = GM_getValue('savedToken', '');
            if (!token) {
                showAlert("❌ Jeton manquant", "error");
                syncBtn.disabled = false;
                ...
                onload: function(response) {
                    try {
                        ...
                        if (data.success) {
                            ...
                            syncBtn.innerHTML = `<span>✅</span> Ok !`;
                        } else {
    ...
    // ==========================================
    // 4. CRÉATION DU BOUTON DANS LE MENU
    // ==========================================
    // Le bouton n'apparaît que si on n'est pas sur ton app PEA Pro
    if (!isPeaApp) {
        GM_registerMenuCommand("📈 Lancer l'extraction PEA Pro", lancerInterfaceEtExtraction);
    }
